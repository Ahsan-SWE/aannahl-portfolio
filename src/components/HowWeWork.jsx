  ...
  return (
    <section className="py-16 px-6 bg-white relative overflow-hidden border-b border-gray-100">
      ...
      <div className="max-w-[85rem] mx-auto">
        ...
        >
          <h2 className="text-[11px] text-[#f97316] uppercase tracking-[0.3em] font-extrabold mb-2">
            Our Process
          </h2>
          <h3 className="text-3xl md:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-600 mb-3">
            How We Work
          </h3>
          <p className="text-gray-500 text-sm md:text-base max-w-2xl mx-auto">
            A streamlined, transparent, and results-driven approach.
          </p>
        </motion.div>
          ...
          <div className="space-y-16 md:space-y-20">
            {processSteps.map((step, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-full md:w-1/2 flex flex-col ${isEven ? "md:items-start md:text-left" : "md:items-end md:text-right"}`}
                  >
                    <motion.span
                      variants={textReveal}
                      className="text-6xl md:text-7xl font-black text-transparent [-webkit-text-stroke:2px_#f3f4f6] group-hover:[-webkit-text-stroke:2px_#f97316] transition-all duration-500 mb-4 leading-none"
                    >
                      {step.id}
                    </motion.span>
                    ...
                    <motion.div
                      variants={textReveal}
                      className="w-14 h-14 bg-white border border-gray-100 shadow-[0_4px_15px_rgba(0,0,0,0.04)] rounded-2xl flex items-center justify-center p-2.5 mb-4 group-hover:-translate-y-1 transition-transform duration-300"
                    >
                      <img
                        src={step.img}
                        alt={`${step.title} Icon`}
                    ...
                    <motion.h4
                      variants={textReveal}
                      className="text-xl md:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-900 via-gray-700 to-gray-900 mb-3 bg-[length:200%_auto] animate-gradient"
                    >
                      {step.title}
                    </motion.h4>
                    ...
                    <motion.p
                      variants={textReveal}
                      className="text-gray-500 leading-relaxed font-medium mb-5 text-sm"
                    >
                      {step.desc}
                    </motion.p>
                    ...
                    <motion.div
                      variants={textReveal}
                      className={`flex flex-wrap gap-2 ${isEven ? "justify-start" : "justify-start md:justify-end"}`}
                    >
                      {step.tags.map((tag, idx) => (
                        <span
                          key={idx}
                          className="bg-gray-50 border border-gray-200 text-gray-500 group-hover:border-[#f97316]/30 group-hover:text-[#f97316] transition-colors text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-md"
                        >
                          {tag}
                        </span>
                      ))}
